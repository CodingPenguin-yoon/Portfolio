...
| 주장                                                      | 상태            | 근거                                                                                                                                                                                                                                          | 포트폴리오 표현                                                                        |
| --------------------------------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| Public GitHub 프로젝트 등록과 main 배포 요청              | Verified        | `backend/src/heimdall/projects/service.py:34-42`, `backend/src/heimdall/deployments/service.py:153-177`; `backend/tests/test_git_client.py:22-30`, `backend/tests/test_deployment_service.py:88-116`                                          | 저장소의 main commit을 single-host Docker preview로 배포한다.                          |
| 정확한 commit checkout 후 Docker image build              | Verified        | `backend/src/heimdall/git/client.py:74-100`, `backend/src/heimdall/runtime/service.py:74-99`; `backend/tests/integration/test_git_checkout.py`, `backend/tests/test_deployment_service.py:88-116`                                             | 요청 시 고정한 commit과 설정 snapshot으로 generation candidate를 빌드한다.             |
| 단일·다중 서비스 generation candidate                     | Verified        | `backend/src/heimdall/runtime/docker.py:99-251`, `backend/src/heimdall/runtime/gateway_config.py:7-31`; `backend/tests/test_docker_runtime.py:299-348`                                                                                        | 세대별 network와 고유 service alias를 사용해 후보 실행 환경을 분리한다.                |
| health check와 실제 route probe 뒤 current 승격           | Verified        | `backend/src/heimdall/runtime/docker.py:229-251`, `backend/src/heimdall/runtime/gateway.py:151-214`; `backend/tests/test_docker_runtime.py:299-348`, `backend/tests/test_nginx_gateway.py:304-345`                                            | service health와 Nginx route가 모두 정상일 때만 current metadata를 전환한다.           |
| Nginx 설정 검증과 atomic activation                       | Verified        | `backend/src/heimdall/runtime/gateway.py:151-214`, `:699-703`; `backend/tests/test_nginx_gateway.py:304-345`                                                                                                                                  | `nginx -t`, atomic config replace, reload, route probe 순으로 활성화한다.              |
| 활성화 실패 시 직전 정상 경로 복원                        | Verified        | `backend/src/heimdall/runtime/gateway.py:215-257`; `backend/tests/test_nginx_gateway.py:347-392`                                                                                                                                              | 실패한 후보를 운영 트래픽에 노출하지 않고 last-known-good 설정과 이전 세대를 유지한다. |
| Worker 재시작 reconciliation                              | Verified        | `backend/src/heimdall/runtime/reconciliation_service.py:34-125`; `backend/tests/test_runtime_reconciliation.py:258-300`                                                                                                                       | Control DB, Nginx marker, Docker label을 비교하고 불확실하면 삭제보다 보존을 선택한다. |
| exact candidate cleanup과 이전 세대 회수                  | Verified        | `backend/src/heimdall/runtime/docker.py:312-324`, `:450-469`, `backend/src/heimdall/runtime/gateway.py:505-524`; `backend/tests/test_docker_runtime.py:350-384`                                                                               | 실패한 배포 ID와 label이 일치하는 후보만 정리하고, 이전 세대는 성공 확정 뒤 회수한다.  |
| SSE 이벤트·서비스 로그와 민감정보 마스킹                  | Verified        | `backend/src/heimdall/runtime/docker_logs.py:109-192`, `backend/src/heimdall/runtime/deployment_diagnostics.py:62-99`; `backend/tests/test_deployment_diagnostics.py:62-106`, `backend/tests/test_service_logs.py:119-226`                    | 배포 상태와 bounded diagnostics를 제공하되 DB·사용자 secret은 마스킹한다.              |
...
| 프로젝트별 PostgreSQL DB와 login role 생성                | Verified        | `backend/src/heimdall/project_database/service.py:38-160`, `backend/src/heimdall/project_database/provisioner.py:20-142`; `backend/tests/test_project_database_service.py:139-189`, `backend/tests/test_project_database_provisioner.py:4-83` | PostgreSQL에서 프로젝트별 DB·login role·schema 권한을 만들고 접속을 검증한다.          |
| deploy-time database credential injection                 | Verified        | `backend/src/heimdall/runtime/docker.py:599-633`; `backend/tests/test_docker_runtime.py:193-255`                                                                                                                                              | DB 접근을 선언한 서비스에만 배포 시점의 연결 정보와 read-only secret을 전달한다.       |
...
### 2026-08-17 현재 저장소 집중 검증

- 대상: `/Users/yoon/03_projects/04_my_vm_proxmox/heimdall_final`, HEAD `b4bda10` (읽기 전용 확인 전후 `git status --short`가 모두 비어 있음).
- 코드·기존 테스트 대조: 위 `Verified` 각 행의 정확한 구현 파일과 테스트 위치를 현재 HEAD에서 직접 확인했다. 외부 Git checkout integration, 실제 Docker, 실제 PostgreSQL이 필요한 테스트는 이번 집중 실행에 포함하지 않았으며 코드와 기존 테스트 정의만 확인했다.
- 안전한 단위 테스트만 격리된 Python 3.13 환경에서 실행했다. bytecode와 pytest cache를 비활성화하고 uv cache를 `/private/tmp`에 뒀다.

```sh
env PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src \
  UV_CACHE_DIR=/private/tmp/heimdall-final-uv-cache \
  uv run --isolated --no-project --python 3.13 \
  --with 'fastapi>=0.116,<1' --with 'psycopg[binary,pool]>=3.3,<4' \
  --with 'uvicorn[standard]>=0.35,<1' --with 'httpx2>=2.7,<3' \
  --with 'pytest>=8.4,<10' pytest -p no:cacheprovider -q \
  tests/test_deployment_service.py::test_main_head_is_resolved_and_snapshot_is_saved \
  tests/test_docker_runtime.py::test_candidate_creates_all_containers_and_retries_start_before_port_lookup \
  tests/test_docker_runtime.py::test_docker_candidate_uses_file_mounts_and_service_scoped_managed_values \
  tests/test_nginx_gateway.py::test_gateway_activates_candidate_and_persists_stable_preview \
  tests/test_nginx_gateway.py::test_failed_route_probe_restores_last_known_good_config \
  tests/test_runtime_reconciliation.py::test_automatic_uncertain_reconciliation_preserves_candidate \
  tests/test_docker_runtime.py::test_verified_cleanup_removes_only_exact_project_deployment_resources \
  tests/test_project_database_service.py::test_project_database_is_provisioned_to_active_without_returning_password \
  tests/test_deployment_diagnostics.py::test_collector_redacts_command_output_and_links_each_service
```

결과: `9 passed` (exit 0). 이 결과는 실행한 아홉 단위 테스트에만 해당한다.
...
| 프로젝트 기간 `2025.09 - 2025.12`      | Verified                       | `src/data/resume.ts:78-90`, `src/data/portfolio.ts:381-397`                                                                                             | 팀 프로젝트 기간을 표기하되 재직 기간처럼 표현하지 않는다.                                                                                                     |
...
| CommandPlan을 실행 경계로 둔 설계 판단 | Verified personal contribution | 기존 이력의 담당 역할 `src/data/resume.ts:81-87`, 프로젝트 서술 `src/data/portfolio.ts:391-398`, 구현 `backend-hybrid/app/services/commands.py:105-219` | 팀 성과와 분리해, 개인이 자연어 요청을 직접 실행하지 않고 의도·대상을 CommandPlan으로 정규화한 뒤 Kubernetes 상태 조회·재시작 작업에 연결한 판단으로 한정한다. |
